...
def main() -> None:
    ...
    remember_parser.add_argument("--role", default="user", help="Message role (user/assistant/system)")
    ...
    tenant_parser = subparsers.add_parser("tenant-create", help="Create a new tenant (requires master admin token)")
    ...
    tenant_parser.add_argument("--admin-token", help="Master admin token (default: KYROS_ADMIN_TOKEN env var)")
    
    # audit command
    audit_parser = subparsers.add_parser("audit", help="Verify cryptographic integrity of an agent's memory")
    audit_parser.add_argument("--agent", required=True, help="Agent ID")
    
    # summarize command
    summarize_parser = subparsers.add_parser("summarize", help="Get a compressed summary/history card for an agent")
    ...
    try:
        if args.command == "status":
            ...
            admin_token = args.admin_token or os.getenv("KYROS_ADMIN_TOKEN") or os.getenv("KYROS_JWT_SECRET_KEY")
            if not admin_token:
                print(
                    "Error: No admin token provided. Pass --admin-token or set KYROS_ADMIN_TOKEN env var.",
                    file=sys.stderr
                )
                sys.exit(1)
            ...
            if r.status_code == 200:
                ...
                print("="*60)
                print("WARNING: Copy this API key now. It will never be displayed in plaintext again.")
            else:
