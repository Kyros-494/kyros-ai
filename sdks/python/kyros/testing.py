...
class TestDataGenerator:
    ...
    def bulk_generate(
        ...
        print(f"Success Generated {num_agents} agent profiles")
        return profiles
...
def load_test_data(client: KyrosClient, num_agents: int = 5, memories_per_agent: int = 10) -> None:
    ...
    print(f"\nSuccess Loaded {total_stored} memories for {num_agents} agents")
...
def run_integration_test(client: KyrosClient, agent_id: str = "test-integration") -> dict[str, Any]:
    ...
    for test in results["tests"]:
        status_icon = "Success" if test["status"] == "PASS" else "ERROR"
