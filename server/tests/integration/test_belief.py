...
async def test_belief_propagation_chain(db_session) -> None:
    ...
    await db_session.execute(
        text(
            "INSERT INTO tenants (id, name, email, api_key_hash, created_at) VALUES (:id, :n, :e, :h, NOW()) ON CONFLICT DO NOTHING"
        ),
        {
            "id": tenant_id,
            "n": "belief-test",
            "e": f"{tenant_id.hex[:8]}@t.com",
            "h": tenant_id.hex,
        },
    )
    await db_session.execute(
        text(
            "INSERT INTO agents (id, tenant_id, external_id, created_at) VALUES (:id, :tid, :eid, NOW()) ON CONFLICT DO NOTHING"
        ),
...
async def test_propagation_respects_max_depth(db_session) -> None:
    ...
    for i, fid in enumerate(fact_ids):
        await db_session.execute(
            text(
                "INSERT INTO semantic_memories (id, agent_id, tenant_id, subject, predicate, object, confidence, embedding, created_at, updated_at) VALUES (:id, :aid, :tid, :s, :p, :o, 0.9, :emb, NOW(), NOW()) ON CONFLICT DO NOTHING"
            ),
            {
                "id": fid,
                "aid": agent_id,
