...
class TestFreshnessCalculation:
    def test_brand_new_memory_is_fully_fresh(self):
        now = _now()
    ...
    def test_high_decay_rate_decays_fast(self):
        now = _now()
    ...
    def test_low_decay_rate_stays_fresh_long(self):
        now = _now()
    ...
    def test_zero_decay_rate_never_decays(self):
        now = _now()
...
class TestFreshnessEvaluation:
    def test_new_memory_is_fresh(self):
    ...
    def test_old_market_data_is_critical(self):
        result = evaluate_freshness(
    ...
    def test_moderately_old_preference_is_warning(self):
        result = evaluate_freshness(
    ...
    def test_identity_stays_fresh_for_months(self):
        result = evaluate_freshness(
...
class TestDecayRateAssignment:
    def test_assigns_correct_rate_for_category(self):
        rate = assign_decay_rate("market_data")
        assert rate == 0.5

    def test_assigns_fallback_for_none(self):
        rate = assign_decay_rate(None)
        assert rate == 0.02

    def test_assigns_fallback_for_unknown(self):
...
class TestBatchUpdate:
    def test_batch_updates_freshness(self):
        now = _now()
        memories = [
            {
                "id": "mem-1",
